...
export class ChatService {
  ...
  constructor() {
    ...
    try {
      ...
      if (apiKey && baseURL) {
        ...
        // Skip connection test to avoid model issues, let real request test it
  ...
  private async testConnection() {
    ...
    try {
      ...
      this.openai = null; // Disable OpenAI if connection fails
    }
  }
  ...
  async generateResponse(history: Array<{role: string, content: string}>): Promise<string> {
    try {
      ...
      // Use dynamic model from environment
      const model = process.env.OPENAI_MODEL || 'gpt-4.1';
  ...
  /**
   * Enhanced fallback response that tries to use transcript content with Cecep personality
   */
  private getEnhancedFallbackResponse(systemContent: string, userQuestion: string): string {
    ...
    // Extract transcript snippet - get more content for better context
    ...
    // Handle specific questions about the video content with Cecep personality
    ...
    // Default enhanced response with Cecep personality and actual video context
    if (transcript) {
  ...
  /**
   * Fallback response when AI is not available - Cecep personality only
   */
